...
namespace CodeInjection::Actions {
    ...
    namespace CallActionController {
        ...
        void CallCallable(uint32_t callableId) {
            ...
            if (callable == _callables.end())
                throw std::runtime_error("CallActionController: Callable not found");
    ...
    public:
        ...
        std::vector<uint8_t> GetBytes(std::shared_ptr<InjectionVariables> vars) {
            ...
            auto relativeAddress =
                GetAddressToControllerFunction() - address - 1 - 4 - 5;  // 5 bytes for the CALL
        ...
        void Perform(std::shared_ptr<InjectionVariables> vars) override {
            ...
            _Log_(
                "CallAction: Writing CALL at 0x{:X} to 0x{:x} (Protected: {})", address, toAddress,
                isWriteProtected
            );
